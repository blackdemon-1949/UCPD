...
void CAD_Init(uint8_t PortNum, USBPD_SettingsTypeDef *pSettings, USBPD_ParamsTypeDef *pParams,  void (*WakeUp)(void))
{
  ...
  LL_GPIO_SetPinMode(GPIOM, LL_GPIO_PIN_0, LL_GPIO_MODE_ANALOG); /* PM0 mode = GP analog => CC1 */
  LL_GPIO_SetPinMode(GPIOM, LL_GPIO_PIN_1, LL_GPIO_MODE_ANALOG); /* PM1 mode = GP analog => CC2 */

  LL_GPIO_SetPinPull(GPIOM, LL_GPIO_PIN_0, LL_GPIO_PULL_NO);
  LL_GPIO_SetPinPull(GPIOM, LL_GPIO_PIN_1, LL_GPIO_PULL_NO);
